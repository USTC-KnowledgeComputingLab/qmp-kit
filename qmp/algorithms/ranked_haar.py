...
class HaarConfig:
    ...
    def main(self, *, model_param: typing.Any = None, network_param: typing.Any = None) -> None:
        ...
        while True:
            ...
            std = 2.5118
            def dist(hop):
                return torch.exp(-(hop- mean)**2/std)
            max_index = original_psi.abs().argmax()
            best_config = configs[max_index]
            diff = torch.bitwise_xor(best_config, configs)
            hopping = torch.zeros_like(diff, dtype=torch.int64)
            for i in range(8):
                hopping = hopping + (diff >> i & 1)
            hopping = hopping.sum(dim=1) / 2
            reweighted_factor = original_psi * dist(hopping)
            max_index = reweighted_factor.abs().argmax()
            target_psi = reweighted_factor / reweighted_factor[max_index]
            logging.info(
...
subcommand_dict["ranked-haar"] = HaarConfig
